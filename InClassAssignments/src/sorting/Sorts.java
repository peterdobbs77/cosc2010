...
public class Sorts {
	...
	/**
	 * Sorts a specified array of int values into ascending order. The
	 * worstTime(n) is O(n * n).
	 *
	 * @param x
	...
	private static int find_pivot(int[] a, int i, int j) {
		return (i + j) / 2;
	}
	...
	/**
	 *  * Sorts a specified int array into ascending order.  *  The worstTime(n)
	 * is O(n log N), where n is the length of the array, and N is the largest
	 *  *  number (in absolute value) of the numbers in the array.  *  *  @param
	...
	public static void radixSort(int[] a) {
		...
		for (int k = 0; k < maxDigits; k++) {
			...
			for (i = 0; i < a.length; i++)
				((LinkedList<Integer>) lists[(int) (a[i] / quotient) % RADIX])
						.add(a[i]);
			i = 0;
