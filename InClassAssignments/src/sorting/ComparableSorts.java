...
public class ComparableSorts {
	...
	/**
	 * Sorts a specified array of objects into ascending order, according to the
	 * element class's implementation of the Comparable interface. The
	...
	public static void insertionSort(Object[] x) {
		for (int i = 1; i < x.length; i++)
			for (int k = i; k > 0
					&& ((Comparable) x[k - 1]).compareTo(x[k]) > 0; k--)
				swap(x, k, k - 1);
